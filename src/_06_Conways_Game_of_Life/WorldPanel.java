...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
    ...
    public WorldPanel(int w, int h, int cpr) {
        ...
        for(int i =0; i < cells.length; i++) {
        	for(int a = 0; a < cells[i].length; a++) {
        		cells[i][a] = new Cell(i*w,a*h, cellSize);
        	}
        }
    }
    ...
    public void randomizeCells() {
    	...
    	for(int i =0; i < cells.length; i++) {
        	for(int a = 0; a < cells[i].length; a++) {
        		cells[i][a].isAlive = ran.nextBoolean();
        	}
    	}
        repaint();
    }
    ...
    public void step() {
        ...
        for(int i = 0; i < cells.length; i++) {
    		for(int a = 0; a < cells[i].length; a++) {
    			int livingNeighborsOut = getLivingNeighbors(cells, i, a);
    			livingNeighbors[i][a] = livingNeighborsOut;
    		}
    	}
        // 8. check if each cell should live or die
        for(int i = 0; i < cells.length; i++) {
    		for(int a = 0; a < cells[i].length; a++) {
    			cells[i][a].liveOrDie(livingNeighbors[i][a]);
    		}
        }
        repaint();
    }
